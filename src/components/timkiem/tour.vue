<template>
      <div class="container">
            <div class="row">
                  <div class="col-4">
                        <router-link :to="{ name: 'timkiemtinh', params: { idTDD: tt.idTinh } }" style="width: 100%;"
                              :key="index" v-for="(tt, index) in alltinh" :id="'tentinh-' + index" type="button"
                              class="btn btn-light" @click="handleButtonClick(tt)">
                              {{ tt.tenTinh }}
                        </router-link>
...
export default {
      ...
      mounted() {
            ...
            console.log('idDD', this.$route.params.idDD)
            ...
            axios.get('http://localhost:3000/api/TDD/motDD/' + urlid)
                  .then((response) => {
