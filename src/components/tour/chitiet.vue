
<template>
      <div class="row">
            <div class="col-8">
                  <!-- About Start -->
                  <div class="container-xxl py-5" :key="index" v-for="(tt, index) in  tour ">
                        <div class="container">
                              <div class="row g-5">
                                    <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.1s" style="min-height: 400px;">
                                          <div class="position-relative h-100">
                                                <img class="img-fluid position-absolute w-100 h-100" :src="tt.hinhT" alt=""
                                                      style="object-fit: cover;">
                                          </div>
                                    </div>
                                    <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.3s">
                                          <h6 class="section-title bg-white text-start text-primary pe-3">Chi tiết tour</h6>
                                          <h4 class="mb-4">Welcome to <span class="text-primary">{{ tt.tenT }}</span></h4>
                                          <p class="mb-4">{{ tt.chitietT }} </p>

                                    </div>
                              </div>
                        </div>
                  </div>
                  <!-- About End -->

                  <div class="container" :key="index" v-for="(tt, index) in  tour ">
                        <!-- <h6 class="section-title bg-white text-center text-primary px-3"></h6> -->
                        <h3 class="mb-5">Những trải nghiệm thú vị trong chương trình</h3>
                        <ul>
                              <li>{{ tt.trainghiem }}</li>
                              <!-- <li>Cung đường Hà Giang “con đường hạnh phúc”</li>
                        <li>Dạo thuyền Sông Nho Quế</li> -->

                        </ul>
                  </div>

                  <div class="container">
                        <h3 class="mb-5">Chương trình tour</h3>
                        <div class="map-route">
                              <section class="section-07 mb-5">
                                    <div class="container">
                                          <div class="timeline-section">
                                                <div>
                                                      <h3 id="day-01">Ngày 1 - NHA TRANG - HÀ NỘI 01 Bữa ăn: (Tối)</h3>
                                                      <div class="excerpt">
                                                            <span class="line"></span>


                                                            <title></title>


                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:black">Quý khách tập trung <a
                                                                                    href="https://www.vietravel.com?utm_source=internalbl&amp;utm_medium=click&amp;utm_campaign=ATLinks"
                                                                                    target="_blank">Vietravel</a> 100
                                                                              Quang Trung, Nha Trang. Xe và hướng dẫn
                                                                              viên đưa đoàn vào sân bay Cam Ranh hỗ trợ
                                                                              khách làm thủ tục đáp chuyến bay đi Hà
                                                                              Nội. Đến sân bay Nội Bài, xe và HDV
                                                                              Vietravel đón Quý khách đi Hà Nội. Xe đưa
                                                                              Quý khách đi tham quan <strong><a
                                                                                          href="https://travel.com.vn/ha-noi/tour-ho-hoan-kiem.aspx?utm_source=internalbl&amp;utm_medium=click&amp;utm_campaign=ATLinks"
                                                                                          target="_blank">Hồ Hoàn
                                                                                          Kiếm</a>&nbsp;</strong>ngắm
                                                                              cầu Thê Húc và đền Ngọc Sơn
                                                                              .</span></span></p>
                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:black">Quý khách dùng cơm tối
                                                                              và nhận phòng khách sạn nghỉ
                                                                              ngơi.&nbsp;Buổi tối, Quý khách tự do dạo
                                                                              36 phố phường Hà Nội, trải nghiệm không
                                                                              gian sôi nổi, náo nhiệt tại Phố Tạ Hiện
                                                                              hay tìm một góc với ly cà phê quan sát phố
                                                                              cổ hẳn cũng rất thú vị.</span></span></p>
                                                            <p style="text-align:right"><span
                                                                        style="font-size:12pt"><strong><span
                                                                                    style="color:red">Nghỉ đêm tại Hà
                                                                                    Nội.&nbsp;</span></strong>&nbsp;</span>
                                                            </p>
                                                            <p style="text-align:right"><span
                                                                        style="font-size:12pt">&nbsp;</span><span
                                                                        style="font-size:12pt"><span
                                                                              style="color:black"><strong>&nbsp;</strong></span></span>
                                                            </p>


                                                      </div>
                                                      <div class="group-services hashCode d-none">
                                                            <div class="item">
                                                                  <i class="icon icon--calendar"></i>
                                                                  <label>Khách sạn</label>
                                                                  <p>VinOasis Phú Quốc</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--fire"></i>
                                                                  <label>Bữa ăn</label>
                                                                  <p>1 bửa trưa, 1 bửa tối.</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--map"></i>
                                                                  <label>Điểm tham quan</label>
                                                                  <p>1 địa điểm</p>
                                                            </div>
                                                      </div>
                                                </div>
                                                <div>
                                                      <h3 id="day-02">Ngày 2 - HÀ NỘI - NINH BÌNH 03 bữa ăn: (Sáng,
                                                            Trưa, Chiều)</h3>
                                                      <div class="excerpt">
                                                            <span class="line"></span>


                                                            <title></title>


                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:#222222">Quý khách ăn sáng và
                                                                              trả phòng. Xe khởi hành đưa Quý khách đi
                                                                              tham quan:</span></span></p>
                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:#222222">-&nbsp;<strong>Lăng
                                                                                    Hồ Chủ Tịch</strong>&nbsp;(không
                                                                              viếng vào thứ 2, thứ 6 hàng tuần và giai
                                                                              đoạn bảo trì định kì hàng năm 15/6 – 15/8)
                                                                              tham quan và tìm hiểu cuộc đời và sự
                                                                              nghiệp của vị cha già dân tộc
                                                                              tại&nbsp;<em>Nhà Sàn Bác Hồ, Bảo Tàng Hồ
                                                                                    Chí Minh, Chùa Một
                                                                                    Cột.</em></span></span></p>
                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:#222222">Tiếp tục hành trình,
                                                                              Quý khách </span><span style="color:black">khởi
                                                                              hành
                                                                              đi&nbsp;<strong>Ninh
                                                                                    Bình&nbsp;</strong>-&nbsp;<em>vùng
                                                                                    đất mệnh danh là “Nơi mơ đến, chốn
                                                                                    mong về” với nhiều di tích văn hóa,
                                                                                    thiên nhiên vô cùng đặc
                                                                                    sắc.</em></span></span></p>
                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:black">Đến nơi, quý khách
                                                                              tham quan:</span></span></p>
                                                            <ul>
                                                                  <li style="text-align:justify"><span
                                                                              style="font-size:12pt"><strong><span
                                                                                          style="color:black">Tuyệt Tịnh
                                                                                          Cốc -</span></strong><span
                                                                                    style="color:black">&nbsp;nằm giữa
                                                                                    mảnh đất cố đô Hoa Lư (Ninh Bình),
                                                                                    nơi đây có động Am Tiên ẩn mình giữa
                                                                                    lưng chừng núi được mệnh danh là
                                                                                    “thiên đường nơi hạ
                                                                                    giới”.</span></span></li>
                                                                  <li style="text-align:justify"><span
                                                                              style="font-size:12pt"><strong><span
                                                                                          style="color:black">Khu <a
                                                                                                href="https://travel.com.vn/?utm_source=internalbl&amp;utm_medium=click&amp;utm_campaign=ATLinks"
                                                                                                target="_blank">du
                                                                                                lịch</a> Thung
                                                                                          Nham:</span></strong><span
                                                                                    style="color:black"> tham quan miệt
                                                                                    vườn – thưởng thức hoa quả tự hái
                                                                                    tại vườn (các loại hoa quả tùy theo
                                                                                    mùa); check-in đảo hoa “Thung Nham’s
                                                                                    flower Paradise”; tìm hiểu câu
                                                                                    chuyện về Cây đa di chuyển, Linh
                                                                                    Thần Miếu và Đền gối đại – khu tâm
                                                                                    linh với nhiều di tích gắn liền 2
                                                                                    thời Vua Đinh, Vua Lê; Quý khách
                                                                                    xuống thuyền đi tham quan “trái tim
                                                                                    di sản”- vườn chim Thung Nham nơi cư
                                                                                    trú và sinh sống của khoảng 40.000
                                                                                    con, thuộc 46 loài như cò, vạc,
                                                                                    diệp, le le, … </span></span></li>
                                                            </ul>
                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="color:black">Quý khách thưởng thức
                                                                              đặc sản Ninh Bình như dê nướng, dê
                                                                              hấp,<strong> …</strong></span><span
                                                                              style="color:black"> và nhận phòng khách
                                                                              sạn nghỉ ngơi. Buổi tối, Quý khách tự do
                                                                              khám phá&nbsp;<strong>Phố cổ Hoa
                                                                                    Lư</strong>,&nbsp;<em>một không gian
                                                                                    check-in cổ, đẹp, trầm mặc đẹp trở
                                                                                    nên lung linh, huyền ảo hơn với sắc
                                                                                    màu của những chiếc đèn lồng và Bảo
                                                                                    Tháp trên Hồ Kỳ Lân; trải nghiệm
                                                                                    nhiều hoạt động trò chơi dân gian và
                                                                                    nhiều loại hình văn hóa nghệ thuật
                                                                                    từ dân tộc như múa rối nước, nhảy
                                                                                    múa Tắc Xình, hát xẩm,.. đến những
                                                                                    buổi trình diễn acoustic sẽ được
                                                                                    diễn ra vào các buổi tối ngày cuối
                                                                                    tuần.&nbsp;&nbsp;</em></span></span>
                                                            </p>
                                                            <p style="text-align:right"><span
                                                                        style="font-size:12pt"><strong><span
                                                                                    style="color:red"><strong><span
                                                                                                style="font-size:12.0pt"><span
                                                                                                      style="color:red">Nghỉ
                                                                                                      đêm tại Ninh
                                                                                                      Bình.&nbsp;</span></span></strong>&nbsp;</span></strong></span>
                                                            </p>


                                                      </div>
                                                      <div class="group-services hashCode d-none">
                                                            <div class="item">
                                                                  <i class="icon icon--calendar"></i>
                                                                  <label>Khách sạn</label>
                                                                  <p>VinOasis Phú Quốc</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--fire"></i>
                                                                  <label>Bữa ăn</label>
                                                                  <p>1 bửa trưa, 1 bửa tối.</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--map"></i>
                                                                  <label>Điểm tham quan</label>
                                                                  <p>1 địa điểm</p>
                                                            </div>
                                                      </div>
                                                </div>
                                                <div>
                                                      <h3 id="day-03">Ngày 3 - NINH BÌNH - TRÀNG AN - BÁI ĐÍNH – HANG
                                                            MÚA 03 Bữa ăn: (Sáng, Trưa, Tối)</h3>
                                                      <div class="excerpt">
                                                            <span class="line"></span>


                                                            <title></title>


                                                            <p><span style="font-size:12pt"><span style="color:black">Quý
                                                                              khách ăn sáng và
                                                                              trả phòng khách sạn. Xe đưa Quý khách đi
                                                                              tham quan:</span></span></p>
                                                            <ul>
                                                                  <li style="text-align:justify"><span
                                                                              style="font-size:12pt"><strong><span
                                                                                          style="color:black"><a
                                                                                                href="https://travel.com.vn/ninh-binh/tour-chua-bai-dinh.aspx?utm_source=internalbl&amp;utm_medium=click&amp;utm_campaign=ATLinks"
                                                                                                target="_blank">Chùa Bái
                                                                                                Đính</a>,</span></strong><span
                                                                                    style="color:black">&nbsp;một quần
                                                                                    thể chùa với nhiều kỷ lục Việt Nam
                                                                                    như pho tượng phật Di Lặc bằng đồng
                                                                                    nặng 80 tấn, hành lang với 500 tượng
                                                                                    vị La Hán, tòa Bảo Tháp cao
                                                                                    99m…&nbsp;</span></span></li>
                                                                  <li style="text-align:justify"><span
                                                                              style="font-size:12pt"><strong><span
                                                                                          style="color:black">Khu Du
                                                                                          Lịch <a
                                                                                                href="https://travel.com.vn/ninh-binh/tour-trang-an.aspx?utm_source=internalbl&amp;utm_medium=click&amp;utm_campaign=ATLinks"
                                                                                                target="_blank">Tràng
                                                                                                An</a>:</span></strong><span
                                                                                    style="color:black">&nbsp;Quý khách
                                                                                    lên thuyền truyền thống đi tham quan
                                                                                    thắng cảnh hệ thống núi đá vôi hùng
                                                                                    vĩ và các thung lũng ngập nước,
                                                                                    thông với nhau bởi các dòng suối tạo
                                                                                    nên các hang động ngập nước quanh
                                                                                    năm. Điểm xuyến trong không gian
                                                                                    hoang sơ, tĩnh lặng là hình ảnh rêu
                                                                                    phong, cổ kính của các mái đình,
                                                                                    đền, phủ nằm nép mình dưới chân các
                                                                                    dãy núi cao.</span></span></li>
                                                                  <li style="text-align:justify"><span
                                                                              style="font-size:12pt"><strong><span
                                                                                          style="color:black">Hang Múa –
                                                                                    </span></strong><span
                                                                                    style="color:black">thiên đường sống
                                                                                    ảo của giới trẻ<strong>,
                                                                                    </strong>nơi đây toát lên một vẻ đẹp
                                                                                    “ma mị” đầy mê hoặc với các du khách
                                                                                    vào buổi xế chiều, đặc biệt vào
                                                                                    tháng 6,7&nbsp; du khách sẽ chiêm
                                                                                    ngưỡng bạt ngàn màu “sen hồng” bên
                                                                                    những ngọn núi hùng vĩ.
                                                                              </span></span></li>
                                                            </ul>
                                                            <p style="text-align:right"><span
                                                                        style="font-size:12pt"><strong><span
                                                                                    style="color:red">Nghỉ đêm tại Ninh
                                                                                    Bình.&nbsp;</span></strong></span>
                                                            </p>


                                                      </div>
                                                      <div class="group-services hashCode d-none">
                                                            <div class="item">
                                                                  <i class="icon icon--calendar"></i>
                                                                  <label>Khách sạn</label>
                                                                  <p>VinOasis Phú Quốc</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--fire"></i>
                                                                  <label>Bữa ăn</label>
                                                                  <p>1 bửa trưa, 1 bửa tối.</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--map"></i>
                                                                  <label>Điểm tham quan</label>
                                                                  <p>1 địa điểm</p>
                                                            </div>
                                                      </div>
                                                </div>
                                                <div>
                                                      <h3 id="day-04">Ngày 4 - NINH BÌNH - HÀ NAM – HÀ NỘI 03 Bữa ăn:
                                                            (Sáng, Trưa, Tối)</h3>
                                                      <div class="excerpt">
                                                            <span class="line"></span>


                                                            <title></title>


                                                            <p style="text-align:justify"><span style="font-size:12pt">Quý
                                                                        khách ăn sáng và trả
                                                                        phòng khách sạn. Xe đưa Quý khách đi tham
                                                                        quan:</span></p>
                                                            <ul>
                                                                  <li style="text-align: justify;"><strong>Địa Tạng Phi
                                                                              Lai Tự</strong> còn được biết đến với tên
                                                                        gọi là Chùa Đùng với ý nghĩa Đức Địa Tạng Vương
                                                                        Bồ Tát ghé đến đây. Ngôi chùa yên tĩnh nằm trên
                                                                        những con dốc phủ kín cây thông, có nhiều tượng,
                                                                        vườn cây và lối đi uốn lượn, là một trong những
                                                                        địa điểm <a
                                                                              href="https://travel.com.vn/?utm_source=internalbl&amp;utm_medium=click&amp;utm_campaign=ATLinks"
                                                                              target="_blank">du lịch</a> tâm linh nổi
                                                                        tiếng của tỉnh Hà Nam được nhiều du khách lựa
                                                                        chọn ghé thăm vãn cảnh, tham quan và tìm kiếm sự
                                                                        thanh bình, thảnh thơi bên lề cuộc sống tấp nập.
                                                                  </li>
                                                                  <li style="text-align: justify;"><span
                                                                              style="font-size:12pt"><strong>Quần thể
                                                                                    danh thắng chùa Tam Chúc, Hà
                                                                                    Nam</strong>: điểm du lịch tâm linh
                                                                              lớn nhất Việt Nam. Quý khách lên thuyền đi
                                                                              tham quan đình Tam chúc, chụp ảnh lưu niệm
                                                                              tại Cây cầu đá bắc ra lòng hồ "Cầu Xuân”.
                                                                              Đến Chùa Tam Chúc, tham quan và chiêm bái
                                                                              tại các khu kiến trúc đặc trưng theo trục
                                                                              thần hoàng: Điện Quán Thế Âm - Điện Giáo
                                                                              Chủ - Điện Tam Thế. Trên đường tàu di
                                                                              chuyển Quý khách có thể nhìn ngắm ánh
                                                                              hoàng hôn đang buông dần tại Hồ Tam Chúc
                                                                              với 6 ngọn núi sừng sững giữa lòng hồ,
                                                                              tương truyền là 6 quả chuông của nhà trời
                                                                              đưa xuống (tiền lục nhạc). Ngoài ra, Quý
                                                                              khách có cơ hội chiêm ngưỡng các báu vật
                                                                              quý tại chùa: Cây bồ đề, Thiên Thạch Mặt
                                                                              Trăng và Vạc Đồng.</span></li>
                                                            </ul>
                                                            <p style="text-align:justify"><span style="font-size:12pt">Xe đưa
                                                                        Quý khách về Hà
                                                                        Nội dùng cơm chiều tại nhà hàng địa phương và
                                                                        nhận phòng nghỉ đêm. Buổi tối, Quý khách tự do
                                                                        khám phá Hà Nội 36 phố phường.</span></p>
                                                            <p style="text-align:right"><span
                                                                        style="font-size:12pt"><strong>Nghỉ đêm tại Hà
                                                                              Nội</strong></span></p>


                                                      </div>
                                                      <div class="group-services hashCode d-none">
                                                            <div class="item">
                                                                  <i class="icon icon--calendar"></i>
                                                                  <label>Khách sạn</label>
                                                                  <p>VinOasis Phú Quốc</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--fire"></i>
                                                                  <label>Bữa ăn</label>
                                                                  <p>1 bửa trưa, 1 bửa tối.</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--map"></i>
                                                                  <label>Điểm tham quan</label>
                                                                  <p>1 địa điểm</p>
                                                            </div>
                                                      </div>
                                                </div>
                                                <div>
                                                      <h3 id="day-05">Ngày 5 - HÀ NỘI - SÂN BAY NỘI BÀI - TP NHA TRANG
                                                            01 Bữa ăn: (Ăn Sáng, trưa tự túc)</h3>
                                                      <div class="excerpt">
                                                            <span class="line"></span>


                                                            <title></title>


                                                            <p style="text-align:justify"><span style="font-size:12pt"><span
                                                                              style="font-size:11.5pt"><span
                                                                                    style="color:#212121">Đoàn dùng bữa
                                                                                    sáng tại khách sạn, tự do nghỉ ngơi
                                                                                    đến giờ khởi hành ra sân bay làm thủ
                                                                                    &nbsp;tục đáp chuyến bay về lại TP
                                                                                    Nha Trang. Đến sân bay Cam Ranh, xe
                                                                                    đón đoàn về lại điểm hẹn ban
                                                                                    đầu.</span></span></span></p>
                                                            <p style="text-align:center"><span
                                                                        style="font-size:12pt"><strong><span
                                                                                    style="color:red">KẾT THÚC CHƯƠNG
                                                                                    TRÌNH, TẠM BIỆT QUÝ
                                                                                    KHÁCH!</span></strong></span></p>
                                                            <p><span style="font-size:12pt"><strong><span
                                                                                    style="color:black">*Lưu ý
                                                                                    :</span></strong><br>
                                                                        <span style="color:black">- Hành trình có thể
                                                                              thay đổi thứ tự điểm đến tùy vào điều kiện
                                                                              thực tế.&nbsp;<br>
                                                                              - Lịch trình tham quan (tắm biển, ngắm
                                                                              hoa, trải nghiệm,...) rất dễ bị ảnh hưởng
                                                                              bởi thời tiết. Đây là trường hợp bất khả
                                                                              kháng mong Quý khách hiểu và thông
                                                                              cảm.<br>
                                                                              - Khách Sạn có thể ở xa trung tâm thành
                                                                              phố vào các mùa Cao Điểm.<br>
                                                                              - Vì những yếu tố khách quan trong giai
                                                                              đoạn này, điểm tham quan có thể đóng cửa
                                                                              và được thay bằng điểm khác phù hợp với
                                                                              chương trình.</span></span></p>


                                                      </div>
                                                      <div class="group-services hashCode d-none">
                                                            <div class="item">
                                                                  <i class="icon icon--calendar"></i>
                                                                  <label>Khách sạn</label>
                                                                  <p>VinOasis Phú Quốc</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--fire"></i>
                                                                  <label>Bữa ăn</label>
                                                                  <p>1 bửa trưa, 1 bửa tối.</p>
                                                            </div>
                                                            <div class="item">
                                                                  <i class="icon icon--map"></i>
                                                                  <label>Điểm tham quan</label>
                                                                  <p>1 địa điểm</p>
                                                            </div>
                                                      </div>
                                                </div>
                                                <div>
                                                </div>
                                          </div>
                                    </div>
                              </section>
                        </div>
                        <br>
                        <h4>Hướng dẫn viên</h4>
                        <p>Hướng Dẫn Viên (HDV) sẽ liên lạc với Quý Khách khoảng 1-2 ngày trước khi khởi hành để sắp xếp giờ
                              đón và
                              cung
                              cấp các thông tin cần thiết cho chuyển đi.</p>
                  </div>
                  <div class="container">
                        <h4>Chi phí dự tính</h4>
                        <p style="font-weight: bolder;">Vận chuyển:
                        <p>Xe đời mới máy lạnh đón khách theo chương trình Xe tham quan (xe 16 chỗ, 29 chỗ) tùy theo số lượng
                              khách
                              thực
                              tế trên chuyến đi.</p>
                        </p>
                        <p style="font-weight: bolder;">Lưu trú:
                        <p>Khách Sạn: 1 Sao, 2 Sao. Phòng: 2-3 khách/phòng. Trường hợp trong đợt khởi hành có khách lẻ ghép,
                              Công ty
                              sẽ
                              ghép cho khách và không phải đóng phụ thu phòng đơn.</p>
                        </p>
                        <p style="font-weight: bolder;">Khác:
                        </p>
                        <ul>
                              <li>Hướng dẫn viên: 1 HDV nhiệt tình, vui vẻ phục vụ đoàn suốt tuyến</li>
                              <li> Nước suối, khăn lạnh,nón ( Nước suối: 1 chai 500ml/ngày/khách. Khăn lạnh: 1 cái/ngày/khách)
                              </li>
                              <li>Thuế VAT</li>
                        </ul>
                  </div>
                  <div class="container">
                        <h3 class="mb-5">Các tour du lịch khác</h3>
                        <div>
                              <!-- Package Start -->
                              <div class="container-xxl py-5">
                                    <div class="container">
                                          <div class="row g-4 justify-content-center">
                                                <div :key="index" v-for="(tt, index) in  tour1 "
                                                      class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
                                                      <div class="package-item">
                                                            <div class="overflow-hidden">
                                                                  <img id="img" class="img-fluid" :src="tt.hinhT" alt="">
                                                            </div>
                                                            <div class="d-flex border-bottom">
                                                                  <!-- <small class="flex-fill text-center border-end py-2"><i
                                                                        class="fa fa-map-marker-alt text-primary me-2"></i>{{  }}</small> -->
                                                                  <small class="flex-fill text-center border-end py-2"><i
                                                                              class="fa fa-calendar-alt text-primary me-2"></i>{{
                                                                                    tt.tgian }}</small>
                                                                  <small class="flex-fill text-center py-2"><i
                                                                              class="fa fa-user text-primary me-2"></i>{{
                                                                                    tt.loaiT
                                                                              }}</small>
                                                            </div>
                                                            <div class="p-4">
                                                                  <h5 class="mb-0">{{ tt.tenT }}</h5>
                                                                  <div class="top">
                                                                        <p class="mb-0">Mã
                                                                              tua: {{ tt.idT }}</p>
                                                                  </div>
                                                                  <div class="top">
                                                                        <p class="mb-0">Khởi hành: {{ tt.khoihanhT }}</p>

                                                                  </div>
                                                                  <div class="top">
                                                                        <p class="mb-0">Nơi khởi hành: {{ tt.noikhoihanh }}
                                                                        </p>
                                                                  </div>
                                                                  <div class="top">
                                                                        <p class="mb-0">Giá: <button type="button"
                                                                                    class="btn btn-warning top">
                                                                                    <p class="mb-0">{{ tt.giaT }}</p>
                                                                              </button></p>
                                                                  </div>
                                                                  <div class="d-flex justify-content-center mb-2 top">
                                                                        <router-link
                                                                              :to="{ name: 'tour', params: { id: tt.idT } }">
                                                                              <button @click="reload"
                                                                                    style="border-radius: 30px 0 0 30px;"
                                                                                    class="btn btn-sm btn-primary px-3 border-end">Chi
                                                                                    tiết</button>
                                                                        </router-link>
                                                                        <!-- <a href="#" class="btn btn-sm btn-primary px-3 border-end"
                                                                        style="border-radius: 30px 0 0 30px;">Chi tiết</a> -->
                                                                        <a href="#" class="btn btn-sm btn-primary px-3"
                                                                              style="border-radius: 0 30px 30px 0;">Đặt
                                                                              tour</a>
                                                                  </div>
                                                            </div>
                                                      </div>
                                                </div>
                                          </div>
                                    </div>
                              </div>
                              <!-- Package End -->

                        </div>
                  </div>
            </div>
            <div class="col-4">
                  <div id="col4">
                        <h4>Lịch khởi hành & giá</h4>
                        <div id="mar">
                              <p>Chọn ngày khởi hành</p>
                              <div class="row">
                                    <button :key="index" v-for="(tt, index) in ngaythang" @click="chonngay(tt.ngaykhoihanh)"
                                          style="padding: 5px; margin-left: 10px; border: 1px solid slateblue;" type="button"
                                          class="btn btn-light col-3">{{ tt.ngaythang }}</button>
                              </div>

                        </div>
                        <div id="congtru" :key="index" v-for="(tt, index) in  km ">
                              <div class="row" style="padding: 10px; border: 1px solid slateblue;margin-top: 10px;">
                                    <!-- <p class="col-4" style="text-align: center;">Người lớn</p> -->
                                    <!-- <p>2300000</p> -->
                                    <input disabled class="col-4" type="text" value="Người lớn" id="gia">
                                    <input disabled class="col-4" type="text" v-model="this.tour[0].nguoilon" id="gia">
                                    <p class="col-4" style="text-align: center;padding-top: 10px;">
                                          <button @click="increaseValue()" id="mau">+</button>
                                          <input id="trangtri" @input="updateForms" type="text" v-model="inputValue">
                                          <button @click="decreaseValue()" id="mau">-</button>
                                    </p>
                              </div>
                              <div class="row" style="padding: 10px;margin-top: 10px;border: 1px solid slateblue; ">
                                    <div class="col-4">
                                          <input disabled type="text" value="Trẻ em" id="gia">
                                          <select class="form-control" v-model="giatre"
                                                style="width: 110px; background-color: aliceblue;">
                                                <option :value="this.tour[0].tred3">Dưới 3 tuổi</option>
                                                <option :value="this.tour[0].tre10">Từ 3 - 10</option>
                                                <!-- <option value="option3"></option> -->
                                          </select>
                                    </div>
                                    <input disabled class="col-4" type="text" v-model="giatre" id="gia">
                                    <p class="col-4" style="text-align: center;">
                                          <button @click="plusValue" id="mau">+</button>
                                          <input id="trangtri" type="text" v-model="ipValue">
                                          <button @click="truValue" id="mau">-</button>
                                    </p>
                              </div>
                              <p style="color: blue;">Liên hệ đặt chỗ</p>
                        </div>
                        <div class="row" id="mar">
                              <h5 class="col-5">Tổng cộng</h5>
                              <input class="col-5" disabled type="text" v-model="tong" readonly>
                        </div>
                        <br>
                        <div class="thanhtoan container">
                              <input disabled type="text" value="Hình thức thanh toán" id="gia">
                              <div class="d-block my-3">
                                    <div class="custom-control custom-radio">
                                          <input id="httt-1" name="httt_ma" type="radio" class="custom-control-input"
                                                required="" v-model="tienmat">
                                          <label class="custom-control-label" for="httt-1">Tiền mặt</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                          <input id="httt-2" name="httt_ma" type="radio" class="custom-control-input"
                                                required="" v-model="vnpay">
                                          <label class="custom-control-label" for="httt-2">Chuyển khoản:
                                                <img width="30px"
                                                      src="data:image/png;base64,"
                                                      alt="" srcset="">
                                          </label>
                                    </div>
                              </div>
                        </div>
                        <div class="row" id="martop">
                              <!-- Button trigger modal -->
                              <button id="tuvan" type="button" class="btn btn-primary col-5 " data-toggle="modal"
                                    data-target="#exampleModal">
                                    Liên hệ tư vấn
                              </button>

                              <!-- Modal -->
                              <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                                    aria-hidden="true">
                                    <div class="modal-dialog">
                                          <div class="modal-content">
                                                <div class="modal-header">
                                                      <h5 class="modal-title" id="exampleModalLabel">Liên hệ tư vấn</h5>
                                                      <button type="button" class="close" data-dismiss="modal"
                                                            aria-label="Close">
                                                            <span aria-hidden="true">&times;</span>
                                                      </button>
                                                </div>
                                                <div class="modal-body">
                                                      <div class="col-xs-12 no-padding">
                                                            <div class="row">
                                                                  <p style="color: coral;">Quý khách vui lòng nhập thông tin
                                                                        liên hệ
                                                                        bên dưới</p>
                                                                  <div class="col-xs-12 v-margin-bottom-15">
                                                                        <label class="control-label">Họ &amp; Tên <span
                                                                                    class="vcolor-danger">*</span></label>
                                                                        <input maxlength="255" type="text"
                                                                              class="form-control " id="customerName"
                                                                              placeholder="">
                                                                  </div>
                                                                  <div class="col-xs-12 v-margin-bottom-15">
                                                                        <label class="control-label">Điện thoại <span
                                                                                    class="vcolor-danger">*</span></label>
                                                                        <input maxlength="100" type="text"
                                                                              class="form-control" id="customerPhone"
                                                                              placeholder="">
                                                                  </div>
                                                                  <div class="col-xs-12 v-margin-bottom-15">
                                                                        <label class="control-label">Email (nếu có)<span
                                                                                    class="vcolor-danger"></span></label>
                                                                        <input maxlength="100" type="text"
                                                                              class="form-control" id="customerEmail"
                                                                              placeholder="">
                                                                  </div>
                                                                  <div class="col-xs-12">
                                                                        <label class="control-label">Yêu cầu khác</label>
                                                                        <textarea maxlength="1000" rows="2" cols="30"
                                                                              class="form-control" id="notes"></textarea>
                                                                  </div>
                                                            </div>
                                                      </div>
                                                </div>
                                                <div class="modal-footer">
                                                      <button type="button" class="btn btn-secondary"
                                                            data-dismiss="modal">Close</button>
                                                      <button type="button" class="btn btn-primary">Gửi yêu cầu</button>
                                                </div>
                                          </div>
                                    </div>
                              </div>

                              <button id="tuvan" @click="test" type="button" class="btn btn-warning col-5">Đặt
                                    tour</button>

                        </div>

                  </div>

            </div>
      </div>
</template>
<style>
#hinh {
      width: 100%;
      height: auto;
}

.top {
      padding-top: 10px;
}

#img {
      width: 100%;
      height: 300px;
}

.login-modal {
      color: #555863
}

#gia {
      font-size: 20px;
      font-weight: 500;
}

.luoi {
      padding-left: 100px;
}

#rong {
      width: 100px;
}

#mau {
      background-color: aliceblue;
      border: aliceblue;
}

#trangtri {
      width: 40px;
      text-align: center;
      border: 1px;
}

#martop {
      margin-top: 30px;
      margin-left: 10px;
}

#tuvan {
      height: 60px;
}

#congtru {
      width: 360px;
      margin-left: 20px;
}

#col4 {
      background-color: aliceblue;
      padding-top: 20px;
}

#mar {
      margin-left: 10px;
}

#gia {
      background-color: aliceblue;
      border: aliceblue;
}
</style>
<script>
import axios from 'axios';
export default {
      data() {
            return {
                  tour: [],
                  tour1: [],
                  LT: [],
                  ngaythang: [],
                  inputValue: 1,
                  ipValue: 0,
                  km: [],
                  tong: 0,
                  giatre: 0,
                  lichtrinh: [],
            }
      },
      computed: {
            tong: function () {
                  if (this.tour.length > 0) {
                        var giaNL = this.tour[0].nguoilon;
                        console.log('gia', giaNL);
                        // Tính toán và trả về kết quả tự động khi giá trị của input thay đổi
                        return (giaNL * this.inputValue) + (this.giatre * this.ipValue);
                  } else {
                        return 0; // Hoặc giá trị mặc định khác tùy theo logic ứng dụng của bạn
                  }
            }
      },
      mounted() {
            var urlid = this.$route.params.id
            var urlidLT = this.$route.params.idLT
            // if (localStorage.getItem('reloaded')) {
            //       // The page was just reloaded. Clear the value from local storage
            //       // so that it will reload the next time this page is visited.
            //       localStorage.removeItem('reloaded');
            // } else {
            //       // Set a flag so that we know not to reload the page twice.
            //       localStorage.setItem('reloaded', '1');
            //       location.reload();
            // }
            console.log('id', urlid)
            axios.get('http://localhost:3000/api/thanhtoan/all/' + urlid)
                  .then((response) => {
                        this.tour = response.data;
                        console.log('tour', this.tour)
                  })
                  .catch((error) => {
                        console.log(error);
                  });
            axios.get('http://localhost:3000/api/thanhtoan/date/' + urlid)
                  .then((response) => {
                        this.ngaythang = response.data;
                        console.log('ngaythang', this.ngaythang)
                  })
                  .catch((error) => {
                        console.log(error);
                  });
            axios.get('http://localhost:3000/api/tour/' + urlid)
                  .then((response) => {
                        this.km = response.data;
                        console.log('KM', this.km)
                  })
                  .catch((error) => {
                        console.log(error);
                  });

            axios.get('http://localhost:3000/api/diadanh/lichtrinh/' + urlid)
                  .then((response) => {
                        this.lichtrinh = response.data;
                        console.log('lichtrinh', this.lichtrinh)
                  })
                  .catch((error) => {
                        console.log(error);
                  });
            axios.get('http://localhost:3000/api/diadanh/lichtrinh/' + urlid + '/' + urlidLT)
                  .then((response) => {
                        this.LT = response.data;
                        console.log('LT', this.LT)
                  })
                  .catch((error) => {
                        console.log(error);
                  });
            axios.get('http://localhost:3000/api/tour/')
                  .then((response) => {
                        this.tour1 = response.data;
                        console.log('tour', this.tour1)
                  })
                  .catch((error) => {
                        console.log(error);
                  });
            axios.get('http://localhost:3000/api/thanhtoan/date/' + urlid)
                  .then((response) => {
                        this.ngaythang = response.data;
                        console.log('ngaythang', this.ngaythang)
                  })
                  .catch((error) => {
                        console.log(error);
                  });
            axios.get('http://localhost:3000/api/tour/' + urlid)
                  .then((response) => {
                        this.km = response.data;
                        console.log('KM', this.km)
                  })
                  .catch((error) => {
                        console.log(error);
                  });

      },
      methods: {
            reload() {
                  window.location.reload();
            },
            test() {
                  var urlid = this.$route.params.id
                  this.$router.push({ name: 'giohang', params: { id: urlid } });
            },
            changeAttribute() {
                  // Thay đổi giá trị isDisabled để thay đổi thuộc tính HTML
                  this.$refs.myElement.setAttribute('active', 'true');
            },
            increaseValue: function () {
                  this.inputValue++;
            },
            decreaseValue: function () {
                  if (this.inputValue > 0) {
                        this.inputValue--;
                  }
            },
            plusValue: function () {
                  this.ipValue++;
            },
            truValue: function () {
                  if (this.ipValue > 0) {
                        this.ipValue--;
                  }
            },
            // Hàm này được gọi khi chọn một option mới
            updateInputValue() {
                  // Cập nhật giá trị của input khi chọn option mới
                  this.$nextTick(() => {
                        // Đặt giá trị của input bằng giá trị của select
                        this.$refs.input.value = this.giatre;
                  });
            },
            chonngay(ngayDuocChon) {
                  // Xử lý ngày được chọn ở đây
                  // console.log("Ngày Được Chọn:", ngayDuocChon);
                  this.selectedNgayKhoiHanh = ngayDuocChon;
                  // console.log("selectedNgayKhoiHanh:", this.selectedNgayKhoiHanh);
                  // Bạn có thể sử dụng ngayDuocChon trong yêu cầu axios hoặc bất kỳ nơi nào bạn cần
            },
      }
}
</script>